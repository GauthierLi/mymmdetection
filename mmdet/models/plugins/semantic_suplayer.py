...
from mmcv.cnn import PLUGIN_LAYERS
from mmcv.runner import BaseModule
from mmdet.utils import upsample_like, show_feature

@PLUGIN_LAYERS.register_module()
class SemanticSup_v1(BaseModule):
    ...
    def single_semantic_loss(self, feature, gt_semantic, indice):
        ...
        # show_feature(feature_, 
        #             f"semsub_feat_mask2former", 
        #             save_dir="/home/gauthierli/code/mmdetection/workdir/semantic_sup8/sem_see",
        #             mode='all')
...
@PLUGIN_LAYERS.register_module()
class SemanticSup_v2(BaseModule):
    """hard supervised"""
    def __init__(self, indices=(0,1,2,3)):
        super().__init__()
        self.eps = 1e-6
    ...
    def single_semantic_loss(self, feature, gt_semantic, indice):
        ...
        for i, gt_sem in enumerate(gt_semantic_):
            feat = feature_[i]
            gt_sem = rearrange(gt_sem, 'c h w->c (h w)')
            coefficient = torch.max((torch.einsum('ce, me->cm', feat.detach(), gt_sem.detach()) + self.eps) \
                                        /(gt_sem.detach().sum(dim=1) + self.eps), 
                                        dim=1)
            coefficient = torch.nn.functional.one_hot(coefficient[1], num_classes=gt_sem.size(0))
            try:
                loss = self.weight_subdice_loss(feat, gt_sem, coefficient)
            except:
                import pdb; pdb.set_trace()
    ...
    def weight_subdice_loss(self, feat, gt, weight):
        tp = torch.einsum('ce, ge->cg', feat, gt)
        fp = torch.einsum('ce, ge->cg', feat, 1 - gt)
        fn = torch.einsum('ce, ge->cg', 1 - feat, gt) 
        subdice = (tp + self.eps) / (tp + fp + self.eps)
        return ((1 - subdice) * weight).mean()
    
@PLUGIN_LAYERS.register_module()
class SemanticSup_v3(BaseModule):
    """hard supervised + tversky"""
    def __init__(self, indices=(0,1,2,3)):
        super().__init__()
        self.eps = 1e-6
        self.indices=indices
    ...
    def forward(self, features, gt_semantics, img_metas):
        ...
        # show_feature(gt_semantic_list[0][0], 
        #             f"semsub_feat_mask2former_gt", 
        #             save_dir="/home/gauthierli/code/mmdetection/workdir/semantic_sup8/sem_see",
        #             mode='all')
        loss = self.multi_apply(self.single_semantic_loss, features, gt_semantic_list, [i for i,_ in enumerate(self.indices)])
        loss = torch.stack(loss).mean()
    ...
    def single_semantic_loss(self, feature, gt_semantic, indice):
        ...
        # show_feature(feature_, 
        #             f"semsub_feat_mask2former", 
        #             save_dir="/home/gauthierli/code/mmdetection/workdir/semantic_sup9/seesee",
        #             mode='mean')
        # show_feature(gt_semantic[0], 
        #             f"semsub_feat_mask2former_gt", 
        #             save_dir="/home/gauthierli/code/mmdetection/workdir/semantic_sup9/seesee",
        #             mode='mean')
    ...
    def weight_subdice_loss(self, feat, gt, weight):
        ...
        fn = torch.einsum('ce, ge->cg', 1 - feat, gt) 
        alpha = 0.25
        subdice = (tp + self.eps) / (tp + alpha * fn + (1 - alpha) * fp + self.eps)
        return ((1 - subdice) * weight).mean()

@PLUGIN_LAYERS.register_module()
class SemanticSup(BaseModule):
    """softmax supervised + tversky"""
    def __init__(self, indices=(0,1,2,3)):
        super().__init__()
        self.eps = 1e-6
        self.indices=indices
        ...
        for i, ele in enumerate(indices):
            channel = 256 * np.power(2, ele)
            layer = nn.Sequential(*[nn.Conv2d(channel, 512, kernel_size=1,stride=1,padding=1),
                                    nn.BatchNorm2d(512), nn.ReLU(),
                                    nn.Conv2d(512, 512, kernel_size=1,stride=1,padding=1),
                                    nn.BatchNorm2d(512), nn.ReLU(),
                                    nn.Conv2d(512, channel, kernel_size=1,stride=1,padding=1)])
            # layer = nn.Conv2d(channel, channel, kernel_size=1,stride=1,padding=0,bias=False)
            self.layers[str(i)] = layer
    ...
    def single_semantic_loss(self, feature, gt_semantic, indice):
        ...
        # show_feature(feature_, 
        #             f"semsub_feat_mask2former", 
        #             save_dir="/home/gauthierli/code/mmdetection/workdir/spatial_bias_3ffn_adp32_8head_additive_semsupv3/seesee",
        #             mode='all')
        # show_feature(gt_semantic[0], 
        #             f"semsub_feat_mask2former_gt", 
        #             save_dir="/home/gauthierli/code/mmdetection/workdir/spatial_bias_3ffn_adp32_8head_additive_semsupv3/seesee",
        ...
        for i, gt_sem in enumerate(gt_semantic_):
            ...
            #                             /(gt_sem.detach().sum(dim=1) + self.eps), 
            #                             dim=1)
            coefficient = torch.softmax((torch.einsum('ce, me->cm', feat, gt_sem) + self.eps) \
                                        /(gt_sem.sum(dim=1) + self.eps), 
                                        dim=1)
            try:
                loss = self.weight_subdice_loss(feat, gt_sem, coefficient)
            except:
