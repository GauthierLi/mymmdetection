...
class MaskFormerHead3(AnchorFreeHead):
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        losses_cls, losses_mask, losses_dice ,losses_proj, losses_gau= multi_apply(
        ...
        loss_dict['loss_gau'] = losses_gau[-1]
        if proj_enable:
            loss_dict['loss_proj'] = losses_proj[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        if proj_enable:
            for loss_cls_i, loss_mask_i, loss_dice_i, loss_proj_i, loss_gau_i in zip(
                    losses_cls[:-1], losses_mask[:-1], losses_dice[:-1], losses_proj[:-1], losses_gau[:-1]):
                loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
                loss_dict[f'd{num_dec_layer}.loss_mask'] = loss_mask_i
                loss_dict[f'd{num_dec_layer}.loss_dice'] = loss_dice_i
                loss_dict[f'd{num_dec_layer}.loss_proj'] = loss_proj_i
                loss_dict[f'd{num_dec_layer}.loss_gau'] = loss_gau_i
                num_dec_layer += 1
        else:
            for loss_cls_i, loss_mask_i, loss_dice_i, loss_gau_i in zip(
                    losses_cls[:-1], losses_mask[:-1], losses_dice[:-1], losses_gau[:-1]):
                loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
                loss_dict[f'd{num_dec_layer}.loss_mask'] = loss_mask_i
                loss_dict[f'd{num_dec_layer}.loss_dice'] = loss_dice_i
                loss_dict[f'd{num_dec_layer}.loss_gau'] = loss_gau_i
